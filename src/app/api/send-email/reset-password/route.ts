...
const ResetPasswordEmailTemplate = ({ resetUrl, userName }: ResetPasswordEmailTemplateProps) => {
  return `
    <!DOCTYPE html>
    <html>
      <head>
        <meta charset="utf-8">
        <title>KomplekIn - Reset Password</title>
        <style>
          body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; }
          .header { background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0; }
          .content { background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px; }
          .button { display: inline-block; background: #ef4444; color: white; padding: 15px 30px; text-decoration: none; border-radius: 8px; font-weight: bold; }
          ...
          .security-box { background: #e0f2fe; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #3b82f6; }
        </style>
      </head>
      <body>
        <div class="header">
        ...
        <div class="content">
          ...
          <p>Kami menerima permintaan untuk mereset password akun KomplekIn Anda. Jika Anda yang membuat permintaan ini, silakan klik tombol di bawah untuk membuat password baru.</p>
          
          <div class="info-box">
            <h3 style="margin: 0 0 15px 0; color: #1f2937;">Reset Password</h3>
          ...
          <div class="security-box">
            <p style="margin: 0; font-size: 14px; color: #1e40af;"><strong>Penting untuk Keamanan:</strong></p>
            <ul style="margin: 10px 0; padding-left: 20px; font-size: 14px;">
              <li>Link reset password ini akan kedaluwarsa dalam 1 jam</li>
              <li>Jika Anda tidak meminta reset password, abaikan email ini</li>
              <li>Password lama Anda masih aktif sampai Anda membuat yang baru</li>
              <li>Setelah reset, login dengan password baru Anda</li>
            </ul>
          </div>

          <p style="margin-top: 30px;">Jika Anda tidak meminta reset password, silakan abaikan email ini dan password Anda akan tetap aman.</p>
...
export async function POST(request: Request) {
  try {
    const { resetUrl, email, userName } = await request.json();
    ...
    const { data, error } = await resend.emails.send({
      ...
      to: [email],
      subject: 'Reset Password - KomplekIn',
